...
static int __devinit chd_pci_reserve_mem(struct crystalhd_adp *pinfo)
{
	...
	if (rc) {
		printk(KERN_ERR "No valid mem region bar0...\n");
		return -ENOMEM;
	}
#else /*(LINUX_VERSION_CODE < KERNEL_VERSION(4,1,0))*/
	if (!request_mem_region(bar0, i2o_len, "crystalhdbar0")) {
		printk(KERN_ERR "No valid mem region bar0...\n");
		return -ENOMEM;
	}
	...
	if (rc) {
		printk(KERN_ERR "No valid mem region bar2...\n");
		return -ENOMEM;
	}
#else /*(LINUX_VERSION_CODE < KERNEL_VERSION(4,1,0))*/
	if (!request_mem_region(bar2, mem_len, "crystalhdbar2")) {
		printk(KERN_ERR "No valid mem region bar2...\n");
		if (pinfo->i2o_addr) iounmap(pinfo->i2o_addr); pinfo->i2o_addr=NULL;
		return -ENOMEM;
	}
...
static const struct pci_device_id chd_dec_pci_id_table[] = { // DEFINE_PCI_DEVICE_TABLE()
